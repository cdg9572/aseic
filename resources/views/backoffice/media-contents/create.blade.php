...
@php
    ...
    $listParameters = $routeParameters;
    if (request()->filled('category_id')) {
        $listParameters['category_id'] = request()->query('category_id');
    }
@endphp
<div class="board-container"><div class="board-header"><a href="{{ route($context['route'].'.index', $listParameters) }}" class="btn btn-secondary btn-sm"><i class="fas fa-arrow-left"></i> 목록으로</a></div><div class="board-card"><div class="board-card-body"><form action="{{ route($context['route'].'.store', $routeParameters) }}" method="POST" enctype="multipart/form-data" class="bo-compact-form">@csrf @include('backoffice.media-contents._form')<div class="board-form-actions"><button type="submit" class="btn btn-primary"><i class="fas fa-save"></i> 저장</button><a href="{{ route($context['route'].'.index', $listParameters) }}" class="btn btn-secondary">취소</a></div></form></div></div></div>
@endsection
