...
@section('title', 'Steering Committee 신규 등록')
...
<div class="board-container"><div class="board-header"><a href="{{ route('backoffice.steering-committee.index') }}" class="btn btn-secondary btn-sm"><i class="fas fa-arrow-left"></i> 목록으로</a></div><div class="board-card"><div class="board-card-body">
<form action="{{ route('backoffice.steering-committee.store') }}" method="POST" class="bo-compact-form">@csrf @include('backoffice.steering-committee._form')<div class="board-form-actions"><button type="submit" class="btn btn-primary"><i class="fas fa-save"></i> 저장</button><a href="{{ route('backoffice.steering-committee.index') }}" class="btn btn-secondary">취소</a></div></form>
</div></div></div>
@endsection
