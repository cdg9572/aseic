...
<div class="board-container">
    ...
    <div class="board-card">
        <div class="board-card-body">
            <form action="{{ route($context['route'] . '.update', $partner) }}" method="POST" enctype="multipart/form-data" class="bo-compact-form">
                @csrf
                @method('PUT')
                @if (request('return_url'))<input type="hidden" name="return_url" value="{{ request('return_url') }}">@endif
                @include('backoffice.homepage-partners._form', ['partner' => $partner])
